...
export const tradeCompanions = mysqlTable("trade_companions", {
  ...
  // 实际入场信息（观察位确认为真实交易后填写）
  actualDirection: varchar("actual_direction", { length: 8 }),
  actualEntryPrice: varchar("actual_entry_price", { length: 32 }),
  actualStopLoss: varchar("actual_stop_loss", { length: 32 }),
  actualTakeProfit: varchar("actual_take_profit", { length: 32 }),
  actualEntryTime: timestamp("actual_entry_time", { mode: 'string' }),
  status: mysqlEnum(['watching', 'active', 'closed', 'cancelled']).default('watching').notNull(),
  createdAt: timestamp("created_at", { mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
