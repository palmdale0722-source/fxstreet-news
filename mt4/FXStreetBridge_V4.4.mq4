...
void SendRequest(string url, string body) {
   ...
   if(res == -1) {
      ...
      string errHint = "";
      if(err == 4060) errHint = " ⚠️ 未在MT4白名单中添加该域名";
      if(err == 5203) errHint = " ⚠️ HTTPS证书不兼容，请将ServerURL改为http://开头";
      Print("[Bridge] 请求失败 → ", url, " | 错误码: ", err, errHint);
   } else {
