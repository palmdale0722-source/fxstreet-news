...
import {
  ...
  Bell, Mail, Send, Eye, EyeOff, Loader2,
} from "lucide-react";
...
function NotifyConfigTab() {
  const utils = trpc.useUtils();
  const { data: config, isLoading } = trpc.notifyConfig.get.useQuery();
  ...
  const saveMutation = trpc.notifyConfig.save.useMutation({
    onSuccess: () => { utils.notifyConfig.get.invalidate(); toast.success("通知配置已保存"); },
    onError: (e) => toast.error(e.message),
  });

  const testEmailMutation = trpc.notifyConfig.testEmail.useMutation({
    onSuccess: () => toast.success("测试邮件已发送，请检查收件箱"),
    onError: (e) => toast.error(`邮件发送失败：${e.message}`),
  });

  const testFeishuMutation = trpc.notifyConfig.testFeishu.useMutation({
    onSuccess: () => toast.success("飞书测试消息已发送"),
    onError: (e) => toast.error(`飞书推送失败：${e.message}`),
  });
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-lg font-bold text-foreground">推送通知设置</h2>
        <p className="text-sm text-muted-foreground mt-0.5">
          当 AI 分析师对交易信号给出「建议执行」或「建议观察」时，自动推送分析结果到您的邮箱或飞书
        </p>
      </div>
      ...
      <div className="rounded-xl border border-border/60 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/60"
          style={{ background: "oklch(0.97 0.01 220 / 0.5)" }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center"
              style={{ background: emailEnabled ? "oklch(0.55 0.15 220)" : "oklch(0.90 0.02 220)" }}>
              <Mail className="w-4 h-4" style={{ color: emailEnabled ? "white" : "oklch(0.55 0.05 220)" }} />
            </div>
            <div>
              <div className="font-semibold text-sm text-foreground">邮件推送</div>
              <div className="text-xs text-muted-foreground">通过 SMTP 发送 HTML 格式的分析报告邮件</div>
            </div>
          </div>
          <button
            onClick={() => setEmailEnabled(!emailEnabled)}
            className="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none"
            style={{ background: emailEnabled ? "oklch(0.55 0.15 220)" : "oklch(0.85 0.02 0)" }}
          >
            <span
        ...
        {emailEnabled && (
          <div className="p-5 space-y-4">
            <div className="grid grid-cols-1 gap-4">
              <div>
                <label className="text-sm font-medium text-foreground mb-1.5 block">收件人邮箱 <span className="text-destructive">*</span></label>
                <input
                  type="email"
                  className="w-full px-3 py-2 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">AI 分析结果将发送到此邮箱</p>
              </div>
            </div>

            <div className="border-t border-border/40 pt-4">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">SMTP 发件配置</p>
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2 sm:col-span-1">
                  <label className="text-sm font-medium text-foreground mb-1.5 block">SMTP 服务器</label>
                  <input
                    className="w-full px-3 py-2 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                ...
                <div>
                  <label className="text-sm font-medium text-foreground mb-1.5 block">端口</label>
                  <input
                    type="number"
                    className="w-full px-3 py-2 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                    placeholder="465"
                ...
                <div className="col-span-2 sm:col-span-1">
                  <label className="text-sm font-medium text-foreground mb-1.5 block">发件人邮箱（SMTP 账号）</label>
                  <input
                    type="email"
                    className="w-full px-3 py-2 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                    placeholder="sender@163.com"
                    value={smtpUser}
                    onChange={e => setSmtpUser(e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-1.5 block">SMTP 密码 / 授权码</label>
                  <div className="relative">
                    <input
                      type={showSmtpPass ? "text" : "password"}
                      className="w-full px-3 py-2 pr-9 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                      placeholder={config?.smtpPassSet ? "（已设置，留空不修改）" : "输入密码或授权码"}
                      value={smtpPass}
              ...
              <div className="flex items-center gap-2 mt-3">
                ...
                <label htmlFor="smtp-secure" className="text-sm text-foreground">
                  使用 SSL/TLS 加密（端口 465 勾选，端口 587 不勾选）
                </label>
              </div>

              <div className="mt-3 p-3 rounded-lg text-xs text-muted-foreground"
                style={{ background: "oklch(0.96 0.01 220 / 0.5)", border: "1px solid oklch(0.90 0.02 220)" }}>
                <strong>常见 SMTP 配置：</strong><br />
                163邮箱：smtp.163.com，端口 465，SSL<br />
                QQ邮箱：smtp.qq.com，端口 465，SSL（需开启 SMTP 并获取授权码）<br />
                Gmail：smtp.gmail.com，端口 587，STARTTLS（需开启两步验证并生成应用密码）
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <button
                ...
                disabled={testEmailMutation.isPending || !smtpHost || !smtpUser || !toEmail}
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium border border-border hover:bg-muted/50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
      ...
      <div className="rounded-xl border border-border/60 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/60"
          style={{ background: "oklch(0.97 0.01 140 / 0.5)" }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center"
              style={{ background: feishuEnabled ? "oklch(0.50 0.15 140)" : "oklch(0.90 0.02 140)" }}>
              <Bell className="w-4 h-4" style={{ color: feishuEnabled ? "white" : "oklch(0.55 0.05 140)" }} />
            </div>
            <div>
              <div className="font-semibold text-sm text-foreground">飞书推送</div>
              <div className="text-xs text-muted-foreground">通过飞书机器人 Webhook 发送富文本卡片消息</div>
            </div>
          </div>
          <button
            onClick={() => setFeishuEnabled(!feishuEnabled)}
            className="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none"
            style={{ background: feishuEnabled ? "oklch(0.50 0.15 140)" : "oklch(0.85 0.02 0)" }}
          >
            <span
              className="inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform"
              style={{ transform: feishuEnabled ? "translateX(22px)" : "translateX(2px)" }}
            />
          </button>
        </div>
          ...
          <div className="p-5 space-y-4">
            <div>
              <label className="text-sm font-medium text-foreground mb-1.5 block">
                飞书机器人 Webhook URL <span className="text-destructive">*</span>
              </label>
              <input
              ...
              />
              {config?.feishuWebhookUrlSet && !feishuWebhookUrl && (
                <p className="text-xs text-muted-foreground mt-1">已设置（留空不修改）：{config.feishuWebhookUrlPreview}</p>
              )}
            </div>

            <div className="p-3 rounded-lg text-xs text-muted-foreground"
              style={{ background: "oklch(0.96 0.01 140 / 0.5)", border: "1px solid oklch(0.90 0.02 140)" }}>
              <strong>如何获取飞书 Webhook URL：</strong><br />
              1. 在飞书群聊中，点击右上角「···」→「设置」→「机器人」→「添加机器人」<br />
              2. 选择「自定义机器人」，填写名称后点击「添加」<br />
              3. 复制生成的 Webhook 地址粘贴到上方输入框<br />
              也可以在「飞书开放平台」创建应用并配置消息推送
            </div>

            <div className="flex justify-end">
              <button
                onClick={() => testFeishuMutation.mutate()}
                disabled={testFeishuMutation.isPending || (!feishuWebhookUrl && !config?.feishuWebhookUrlSet)}
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium border border-border hover:bg-muted/50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {testFeishuMutation.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                发送测试消息
              </button>
            </div>
          </div>
    ...
    <div className="min-h-screen" style={{ background: "oklch(0.97 0.005 280)" }}>
      ...
      <div className="container py-6 max-w-4xl">
        ...
        <div className="bg-background rounded-2xl border border-border/60 p-6">
          ...
          {activeTab === "notify" && <NotifyConfigTab />}
        </div>
      </div>
    </div>
  );
